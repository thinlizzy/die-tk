...
void BallView::moveBall(tk::Point offset) {
	...
	window.invalidate(ball.rect());
}

void BallView::draw() {
	auto rect = ball.rect();
	auto dims = rect.dims();
	window.canvas().fillRect(rect.resize(dims - tk::WDims::squared(drawOff)).add(tk::Point(drawOff / 2, drawOff / 2)),
							 ballColor2); // inner
	window.canvas().fillRect(rect.resize(dims.addHeight(-drawOff * 2)).add(tk::Point().addY(drawOff)),
							 ballColor1); // wide
	window.canvas().fillRect(rect.resize(dims.addWidth(-drawOff * 2)).add(tk::Point().addX(drawOff)),
							 ballColor1); // tall
}
