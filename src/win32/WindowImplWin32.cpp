...
{
    DWORD result = 0;
    if( params.multiSelect_ ) result |= OFN_ALLOWMULTISELECT | OFN_EXPLORER;
...
std::vector<die::NativeString> WindowImpl::selectFile(SelectFile operation, SelectFileParams const & params)
{
    OPENFILENAMEW ofn;
    ...
    ofn.Flags = ofnFlags(params);
    ofn.nFileOffset = ofnLast(params,L'\\');
    ofn.nFileExtension = ofnLast(params,L'.');
    ofn.lpstrDefExt = NULL;

    BOOL ok = operation == SelectFile::save ? GetSaveFileName(&ofn) : GetOpenFileName(&ofn);
