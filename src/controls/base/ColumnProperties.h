...
public:
    ...
    die::NativeString header;
    int width;
    ImageList::Index imageIndex;
    Alignment alignment;
    
    ColumnProperties(): width(50), imageIndex(ImageList::noIndex), alignment(left) {}
    ColumnProperties(die::NativeString const & header, int width = 50, ImageList::Index imageIndex = ImageList::noIndex): 
        header(header), width(width), imageIndex(imageIndex), alignment(left) {}
    ColumnProperties & setHeader(die::NativeString const & header) { this->header = header; return *this; }
